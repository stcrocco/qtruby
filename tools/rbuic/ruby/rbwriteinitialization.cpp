...
void WriteInitialization::acceptUI(DomUI *node)
{
    ...
    m_registeredWidgets.insert(m_mainFormVarName, node->elementWidget()); // register the main widget

    m_mainFormVarName = m_mainFormVarName.mid(0, 1).toLower() + m_mainFormVarName.mid(1);

    const QString widgetClassName = node->elementWidget()->attributeClass();

    m_output << m_option.indent << "def setupUi(" << m_mainFormVarName << ")\n";
    ...
    for (int i=0; i<connections.size(); ++i) {
        ...
        const QString varConn = QLatin1String("@") + connection + QLatin1String("Connection");
        m_output << m_indent << varConn << " = Qt::SqlDatabase::database(" << writeString(connection, m_dindent) << ");\n";
    }
    ...
    for (int i=0; i<m_buddies.size(); ++i) {
        ...
        m_output << m_indent << "@" << name << ".buddy = @" << buddyName << "\n";
    }
    if (m_buddies.size() > 0)
    ...
    m_output << "\n" << m_indent << "retranslateUi(" << m_mainFormVarName << ")\n";
    ...
    if (m_option.autoConnection)
        m_output << "\n" << m_indent << "Qt::MetaObject::connectSlotsByName(" << m_mainFormVarName << ")\n";

    m_output << m_option.indent << "end # setupUi\n\n";

    m_output << m_option.indent << "def setup_ui(" << m_mainFormVarName << ")\n";
    m_output << m_option.indent << "    setupUi(" << m_mainFormVarName << ")\n";
    m_output << m_option.indent << "end\n\n";

//    m_output << m_option.indent << "def retranslateUi(" << varName << ")\n"
    m_output << m_option.indent << "def retranslateUi(" << m_mainFormVarName << ")\n"
...
void WriteInitialization::acceptWidget(DomWidget *node)
{
    ...
    if (m_widgetChain.size() != 1) {
        m_output << m_indent << varName << " = " << m_driver->rubyClassName(m_uic->customWidgetsInfo()->realClassName(className)) <<
          ".new(" << parentWidget << ")\n";
    }
    ...
    if (m_uic->customWidgetsInfo()->extends(className, QLatin1String("QTreeView"))
        ...
        writeProperties(varName + QLatin1String(".header"), QLatin1String("Qt::HeaderView"),
        ...
        for (const QString &headerPrefix : headerPrefixes) {
            ...
            writeProperties(varName + QLatin1String(".") + headerPrefix,
                            QLatin1String("Qt::HeaderView"),
                            headerProperties, WritePropertyIgnoreObjectName);
        }
    }
...
void WriteInitialization::addButtonGroup(const DomWidget *buttonNode, const QString &varName)
{
    ...
    if (!m_buttonGroups.contains(groupName)) {
        const QString className = QLatin1String("Qt::ButtonGroup");
        m_output << m_indent;
        m_output << groupName << " = " << className << ".new(" << m_mainFormVarName << ")\n";
...
{
    const QString className = node->attributeClass();
    const QString varName = toRubyIdentifier(m_driver->findOrInsertLayout(node));
    ...
    m_output << m_indent << varName << " = " << m_driver->rubyClassName(className) << ".new(";

    if (!m_layoutChain.top() && !isGroupBox){
        QString parentWidget = m_driver-> findOrInsertWidget(m_widgetChain.top());
        parentWidget = parentWidget.mid(0,1).toLower() + parentWidget.mid(1);
        if(m_widgetChain.count() != 2) parentWidget.prepend("@");
        m_output << parentWidget;
    }
    m_output << ")\n";
//    if(isGroupBox) {
//      m_output << m_indent << varName << ".alignment = Qt::AlignTop\n";
//    }
...
{
    if (colspan > 1)
        return QLatin1String("Qt::FormLayout::SpanningRole");
    return column == 0 ? QLatin1String("Qt::FormLayout::LabelRole") : QLatin1String("Qt::FormLayout::FieldRole");
}
...
{
    const QString actionName = toRubyIdentifier(m_driver->findOrInsertActionGroup(node));
    QString varName = m_driver->findOrInsertWidget(m_widgetChain.top());
    varName = varName.mid(0, 1).toLower() + varName.mid(1);
    if(m_widgetChain.count() > 2) varName.prepend("@");

    if (m_actionGroupChain.top())
        varName = m_driver->findOrInsertActionGroup(m_actionGroupChain.top());

    m_output << m_indent << actionName << " = Qt::ActionGroup.new(" << varName << ")\n";
...
void WriteInitialization::acceptAction(DomAction *node)
{
    ...
    m_output << m_indent << actionName << " = Qt::Action.new(" << varName << ")\n";
...
void WriteInitialization::acceptActionRef(DomActionRef *node)
{
    ...
    m_actionOut << m_indent << varName << ".addAction(" << rubyActionName << ")\n";
}
                                          ...
                                          unsigned flags)
{
    ...
    if (!m_widgetChain.top()) {
        ...
        //objectName should never be nil, in my opinion, as it's supposed to be
        //a string. However, in QtRuby 4 it could be, so, for the time being,
        //accept both
        m_output << m_indent << "if " << varName << ".objectName.nil? || " << varName << ".objectName.emtpy?\n";
    }
    if (!(flags & WritePropertyIgnoreObjectName)) {
      if(varName.startsWith("@")){
        m_output << m_indent << indent << varName << ".objectName = " << writeString(varName.mid(1), m_dindent) << "\n";
      }
      else {
        m_output << m_indent << indent << varName << ".objectName = " << writeString(varName.mid(0,1).toLower() + varName.mid(1), m_dindent) << "\n";
      }
    }
    if (!m_widgetChain.top()) {
        indent = m_option.indent;
    ...
    for (int i=0; i<lst.size(); ++i) {
        ...
        case DomProperty::Cstring:
            if (propertyName == QLatin1String("buddy") && m_uic->customWidgetsInfo()->extends(className, QLatin1String("QLabel"))) {
                ...
                if (stdset)
                    ...
                    propertyValue = QLatin1String("Qt::ByteArray.new(")
        ...
        case DomProperty::Palette: {
            ...
            const QString paletteName = QString("@") + m_driver->unique(QLatin1String("palette"));
            m_output << m_indent << paletteName << "Qt::Palette.new \n";
        ...
        case DomProperty::String: {
            if (propertyName == QLatin1String("objectName")) {
                ...
                if ((varName.startsWith("@") && v == varName.mid(1)) || 
                    varName == v.mid(0,1).toLower()+ v.mid(1))
                    break;
        ...
        case DomProperty::DateTime: {
            ...
            propertyValue = QString::fromLatin1("Qt::DateTime.new(QDate(%1, %2, %3), QTime(%4, %5, %6))")
                            .arg(dt->elementYear())
                            .arg(dt->elementMonth())
                            .arg(dt->elementDay())
                            .arg(dt->elementHour())
...
QString  WriteInitialization::writeSizePolicy(const DomSizePolicy *sp)
{
    ...
    m_output << m_indent << spName << " = Qt::SizePolicy.new";
    do {
...
QString WriteInitialization::writeFontProperties(const DomFont *f)
{
    ...
    m_output << m_indent << fontName <<  " = Qt::Font.new\n";
    if (f->hasElementFamily() && !f->elementFamily().isEmpty()) {
        m_output << m_indent << fontName << ".pointSize = " << writeString(f->elementFamily(), m_dindent)
            << "\n";
    }
    ...
    if (f->hasElementBold()) {
        m_output << m_indent << fontName << ".bold = "
            << (f->elementBold() ? "true" : "false") << "\n";
    }
    if (f->hasElementItalic()) {
        m_output << m_indent << fontName << ".italic = "
            <<  (f->elementItalic() ? "true" : "false") << "\n";
    }
    if (f->hasElementUnderline()) {
        m_output << m_indent << fontName << ".underline = "
                              ...
                              const DomResourceIcon *i)
{
    if (i->hasElementNormalOff())
        output << indent << iconName << ".addFile(" << writeString(i->elementNormalOff()->text(), indent) << ", Qt::Size(), Qt::Icon::Normal, Qt::Icon::Off)\n";
    if (i->hasElementNormalOn())
        output << indent << iconName << ".addFile(" << writeString(i->elementNormalOn()->text(), indent) << ", Qt::Size(), Qt::Icon::Normal, Qt::Icon::On)\n";
    if (i->hasElementDisabledOff())
        output << indent << iconName << ".addFile(" << writeString(i->elementDisabledOff()->text(), indent) << ", Qt::Size(), Qt::Icon::Disabled, Qt::Icon::Off)\n";
    if (i->hasElementDisabledOn())
        output << indent << iconName << ".addFile(" << writeString(i->elementDisabledOn()->text(), indent) << ", Qt::Size(), Qt::Icon::Disabled, Qt::Icon::On)\n";
    if (i->hasElementActiveOff())
        output << indent << iconName << ".addFile(" << writeString(i->elementActiveOff()->text(), indent) << ", Qt::Size(), Qt::Icon::Active, Qt::Icon::Off)\n";
    if (i->hasElementActiveOn())
        output << indent << iconName << ".addFile(" << writeString(i->elementActiveOn()->text(), indent) << ", Qt::Size(), Qt::Icon::Active, Qt::Icon::On)\n";
    if (i->hasElementSelectedOff())
        output << indent << iconName << ".addFile(" << writeString(i->elementSelectedOff()->text(), indent) << ", Qt::Size(), Qt::Icon::Selected, Qt::Icon::Off)\n";
    if (i->hasElementSelectedOn())
        output << indent << iconName << ".addFile(" << writeString(i->elementSelectedOn()->text(), indent) << ", Qt::Size(), Qt::Icon::Selected, Qt::Icon::On)\n";
}
        ...
        if (i->attributeTheme().isEmpty()) {
            ...
            if (iconHasStatePixmaps(i)) {
                ...
                m_output << themeNameStringVariableC << " = "
                         << themeIconName << "\n";
                m_output << m_indent << "if Qt::Icon::hasThemeIcon("
                         << themeNameStringVariableC
                         << ")\n"
                         << m_dindent << iconName << " = Qt::Icon::fromTheme(" << themeNameStringVariableC << "\n"
                         << m_indent << "\nelse\n";
                writeResourceIcon(m_output, iconName, m_dindent, i);
                m_output << m_indent << "\n";
            } else {
                // Theme, but no state pixmaps: Construct from theme directly.
                m_output << m_indent << iconName
                         << "= Qt::Icon::fromTheme("
                         << themeIconName << ")\n";
            } // Theme, but not state
        }     // >= 4.4
    } else {  // pre-4.4 legacy
        m_output <<  m_indent << iconName << " = " << pixCall(QLatin1String("Qt::Icon.new"), i->text())<< "\n";
    }
    return iconName;
}
...
void WriteInitialization::writeBrush(const DomBrush *brush, const QString &brushName)
{
    ...
    if (style == QLatin1String("LinearGradientPattern") ||
        ...
        if (gradientType == QLatin1String("LinearGradient")) {
            m_output << m_indent << gradientName << "Qt::LinearGradient.new"
            ...
            m_output << m_indent << gradientName << " = Qt::RadialGradient.new"
            ...
            m_output << m_indent << gradientName << " = Qt::ConicalGradient.new"
                << '(' << gradient->attributeCentralX()
                << ", " << gradient->attributeCentralY()
                << ", " << gradient->attributeAngle() << ")\n";
        }

        m_output << m_indent << gradientName << ".spread = Qt::Gradient::"
            << gradient->attributeSpread() << "\n";

        if (gradient->hasAttributeCoordinateMode()) {
            m_output << m_indent << gradientName << ".coordinateMode = Qt::Gradient::"
       ...
       const  QList<DomGradientStop *> stops = gradient->elementGradientStop();
        ...
        m_output << m_indent << brushName << " = Qt::Brush.new("
            << gradientName << ")\n";
        ...
        m_output << m_indent << brushName << ".style = "
            << "Qt::" << style << ")\n";
    }
}
...
void WriteInitialization::acceptTabStops(DomTabStops *tabStops)
{
    ...
    for (int i=0; i<l.size(); ++i) {
        ...
        m_output << m_indent << "Qt::Widget::setTabOrder(@" << lastName << ", @" << name << ")\n";
...
QString WriteInitialization::pixCall(const QString &t, const QString &text) const
{
    ...
    if (const DomImage *image = findImage(text)) {
        if (m_option.extractImages) {
            ...
            return QLatin1String("Qt::Pixmap.new(\":/")
                 + m_generatedClass
...
QString WriteInitialization::disableSorting(DomWidget *w, const QString &varName)
{
    ...
    if (!w->elementItem().isEmpty()) {
        ...
        m_refreshOut << "\n";
        m_refreshOut << m_indent << tempName
            << " = " << varName << "isSortingEnabled?\n";
        m_refreshOut << m_indent << varName << "SortingEnabled = false\n";
    }
    return tempName;
}
...
void WriteInitialization::initializeListWidget(DomWidget *w)
{
    ...
    for (int i = 0; i < items.size(); ++i) {
        ...
        Item item(QLatin1String("Qt::ListWidgetItem"), m_indent, m_output, m_refreshOut, m_driver);
...
void WriteInitialization::initializeTreeWidget(DomWidget *w)
{
    ...
    // columns
    Item item(QLatin1String("Qt::TreeWidgetItem"), m_indent, m_output, m_refreshOut, m_driver);
    ...
    for (int i = 0; i < columns.size(); ++i) {
        ...
        if (const DomProperty *p = properties.value(QLatin1String("text"))) {
            ...
            if (str && str->text().isEmpty())
                m_output << m_indent << varName << ".headerItem().setText(" << i << ", '')\n";
        }
    }
...
//TODO: should this be Qt::TreeWidgetItem?
...
void WriteInitialization::initializeTableWidget(DomWidget *w)
{
    ...
    if (columns.size() != 0) {
        m_output << m_indent << "if " << varName << ".columnCount < " << columns.size() << ")\n"
            << m_dindent << varName << ".columnCount = " << columns.size() << "\n"
            << "end\n";
    }
...
//TODO: should this be Qt::TableWidgetItem?
    ...
    if (rows.size() != 0) {
        m_output << m_indent << "if " << varName << ".rowCount < " << rows.size() << "\n"
            << m_dindent << varName << ".rowCount" << rows.size() << "\n" <<
            "end\n";
    }
...
QString WriteInitialization::trCall(const QString &str, const QString &commentHint) const
{
    ...
    QString result;
    const QString comment = commentHint.isEmpty() ? QString(QLatin1String("nil")) : fixString(commentHint, m_dindent);

    if (m_option.translateFunction.isEmpty()) {
        if (m_option.idBased) {
          //TODO Is this correct? What is qtTrId?
...
void WriteInitialization::acceptConnection(DomConnection *connection)
{
    ...
    m_output << m_indent << "Qt::Object.connect("
        << sender
        << ", "
...
//TODO: should this be removed? What are widgets scripts?
...
QString WriteInitialization::Item::writeSetupUi(const QString &parent, Item::EmptyItemPolicy emptyItemPolicy)
{
    ...
    if (emptyItemPolicy == Item::ConstructItemOnly && m_children.size() == 0) {
        if (m_setupUiData.policy == ItemData::DontGenerate) {
            m_setupUiStream << m_indent << m_itemClassName << ".new" << '(' << parent << ")\n";
                return QString();
        } 
    }
...
void WriteInitialization::Item::writeRetranslateUi(const QString &parentPath)
{
    ...
    m_retranslateUiStream << m_indent << uniqueName << " = " << parentPath << "\n";
